...
function RecoverPassword({ onNavigateBack }) {
  ...
  const handleStep2 = async (e) => {
    ...
    setServerMessage({ type: '', text: '' })
    try {
      await recoverPassword(formData.email, formData.newPassword)
      setServerMessage({
        type: 'success',
        ...
        <div className="bg-white rounded-2xl shadow-custom-xl overflow-hidden">
          <div className="bg-gradient-to-r from-accent to-purple-500 px-8 py-10 text-center">
            ...
            <p className="text-white/70 text-sm">
              {step === 1 ? 'Ingresa tu correo para continuar' : 'Define tu nueva contraseña'}
            </p>
          </div>
          <div className="p-8">
            {serverMessage.text && (
            ...
            {step === 1 ? (
              ...
              <form onSubmit={handleStep2}>
                <div className="mb-5 p-3 rounded-lg bg-blue-50 border border-blue-100 text-blue-700 text-sm">
                  Enviando cambio para: <strong>{formData.email}</strong>
                </div>
                <Input label="Nueva Contraseña" type="password" name="newPassword"
                  ...
                  icon={passwordIcon} maxLength={50} required />
                <Input label="Confirmar Contraseña" type="password" name="confirmarPassword"
                  value={formData.confirmarPassword} onChange={handleChange} onBlur={handleBlur}
            ...
            >
              {step === 2 ? '← Volver a ingresar correo' : '← Volver al inicio de sesión'}
            </button>
          </div>
        </div>
