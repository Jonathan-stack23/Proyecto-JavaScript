...
function AdminServicios() {
  ...
  const openCreate = () => {
    setFormData({ nombre: '', descripcion: '', precio: '', duracion: '', categoria: '', estado: 'activo' })
    setErrors({})
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-text-heading">Gestión de Servicios</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {items.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-10 text-gray-400">Sin servicios registrados</td></tr>
                ) : items.map(s => (
                  <tr key={s.id} className="hover:bg-gray-50/50">
                    <td className="px-5 py-3 text-gray-500">#{s.id}</td>
                    <td className="px-5 py-3">
                      <p className="font-medium text-text-heading">{s.nombre}</p>
                      <p className="text-[11px] text-gray-400 line-clamp-1">{s.descripcion}</p>
                    </td>
                    <td className="px-5 py-3">
                      {s.categoria
                        ? <span className="px-2.5 py-1 rounded-full text-[11px] font-semibold bg-amber-100 text-amber-700">{s.categoria}</span>
                        : <span className="text-gray-300">—</span>}
                    </td>
                    <td className="px-5 py-3 font-semibold text-text-heading">{formatearCOP(s.precio)}</td>
                    <td className="px-5 py-3">{s.duracion || <span className="text-gray-300">—</span>}</td>
